...
func (vmst *VMStorageWithTenantID) InitSearch(qt *querytracer.Tracer, sq *storage.SearchQuery, deadline uint64) (vmselectapi.BlockIterator, error) {
	...
	convertSearchQueryTimestampsToMicroseconds(sq)
...
func (vmst *VMStorageWithTenantID) marshalMetricBlock(dst []byte, src *storage.MetricBlock) []byte {
	...
	// Marshal data block.
	src.Block.MustConvertTimestampsToMilliseconds()
	dst = encoding.MarshalUint32(dst, vmst.accountID)
	dst = encoding.MarshalUint32(dst, vmst.projectID)
...
func (vmst *VMStorageWithTenantID) TagValueSuffixes(qt *querytracer.Tracer, accountID, projectID uint32, tr storage.TimeRange, tagKey, tagValuePrefix string, delimiter byte, maxSuffixes int, deadline uint64) ([]string, error) {
	...
	tr.MinTimestamp = timestampMillisecondsToMicroseconds(tr.MinTimestamp)
	tr.MaxTimestamp = timestampMillisecondsToMicroseconds(tr.MaxTimestamp)
...
func (vmst *VMStorageWithTenantID) RegisterMetricNames(qt *querytracer.Tracer, mrs []storage.MetricRow, deadline uint64) error {
	for i := range mrs {
		mrs[i].Timestamp = timestampMillisecondsToMicroseconds(mrs[i].Timestamp)
	}
...
func convertSearchQueryTimestampsToMicroseconds(sq *storage.SearchQuery) {
	sq.MinTimestamp = timestampMillisecondsToMicroseconds(sq.MinTimestamp)
	sq.MaxTimestamp = timestampMillisecondsToMicroseconds(sq.MaxTimestamp)
}

func timestampMillisecondsToMicroseconds(timestamp int64) int64 {
	if timestamp > math.MaxInt64/1000 {
		return math.MaxInt64
	}
	if timestamp < math.MinInt64/1000 {
