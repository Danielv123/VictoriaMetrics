...
func TestStorageDeletePendingSeries(t *testing.T) {
	...
	assertCountRows := func(count int) {
		...
		search.Init(nil, s, []*TagFilters{tfs}, TimeRange{start.UnixMicro(), math.MaxInt64}, 1e5, noDeadline)
		n := 0
...
func testStorageDeleteSeries(t *testing.T, concurrency int, disablePerDayIndex bool) {
	tr := TimeRange{
		MinTimestamp: time.Date(2026, 1, 15, 0, 0, 0, 0, time.UTC).UnixMicro(),
		MaxTimestamp: time.Date(2026, 1, 15, 23, 59, 59, 999_999_999, time.UTC).UnixMicro(),
	}
...
func TestStorageRowsNotAdded(t *testing.T) {
	...
	minTimestamp = time.Now().Add(-maxBackfillAge - time.Hour).UnixMicro()
	maxTimestamp = minTimestamp + 1000
	f(&options{
...
func TestStorageSearchLabelValues_SingleFilterOnLabelName(t *testing.T) {
	...
	tr := TimeRange{
		MinTimestamp: time.Date(2024, 1, 15, 0, 0, 0, 0, time.UTC).UnixMicro(),
		MaxTimestamp: time.Date(2024, 12, 15, 23, 59, 59, 999_999_999, time.UTC).UnixMicro(),
	}
...
func testStorageAddRowsWithZeroDate(t *testing.T, disablePerDayIndex bool) {
	...
	for day := range numDays {
		...
		mrs[day].Timestamp = int64(day * usecPerDay)
	}
	...
	tr = TimeRange{
		MinTimestamp: usecPerDay,
		MaxTimestamp: 2*usecPerDay - 1,
	}
	...
	for workerID := range concurrency {
		wg.Go(func() {
			for m := time.Month(1); m <= 12; m++ {
				tr := TimeRange{
					MinTimestamp: time.Date(2025, m, 1, 0, 0, 0, 0, time.UTC).UnixMicro(),
					MaxTimestamp: time.Date(2025, m+1, 0, 0, 0, 0, 0, time.UTC).UnixMicro() - 1,
				}
				err := f(workerID, tr)
				if err != nil {
