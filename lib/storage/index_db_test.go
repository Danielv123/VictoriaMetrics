...
func testIndexDBSearchLabelValues(t *testing.T, disablePerDayIndex bool) {
	...
	baseTimestamp := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC).UnixMicro()
	baseDate := uint64(baseTimestamp) / usecPerDay
	tr1d := TimeRange{
		MinTimestamp: baseTimestamp,
		MaxTimestamp: baseTimestamp + usecPerDay - 1,
	}
	tr2d := TimeRange{
		MinTimestamp: baseTimestamp,
		MaxTimestamp: baseTimestamp + 2*usecPerDay - 1,
	}
...
func testIndexDBDeleteSeries(t *testing.T, disablePerDayIndex bool) {
	...
	const metricsPerDay = 1000
	trAllDays := TimeRange{
		MinTimestamp: time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC).UnixMicro(),
		MaxTimestamp: time.Date(2026, 1, days+1, 0, 0, 0, 0, time.UTC).UnixMicro() - 1,
	}
	date0 := uint64(trAllDays.MinTimestamp) / usecPerDay
	allMetricIDs := &uint64set.Set{}
...
func newTestStorage() *Storage {
	s := &Storage{
		...
		maxBackfillAgeUsecs: retentionMax.Microseconds(),
	}
	return s
}
